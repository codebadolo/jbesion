...
/**
 * Form for creating a Fiche Interne.
 * Props:
 *   onSuccess – optional callback after successful creation
 */
export default function FicheInterneForm({ onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        ...
        createFiche({
          ...
          data: {
            ...
            items: items.map(({ _id, ...rest }) => ({
              ...
              montant: rest.montant ? parseFloat(rest.montant) : null,
            })),
          },
        }),
      ).unwrap()
      ...
      if (onSuccess) {
        ...
        navigate(`/fiches-internes/${result.data?.id || result.id}`)
      }
    } catch {
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <label className="form-label mb-0">
            Articles / Besoins <span className="text-red-500">*</span>
          </label>
