...
function StatsCards({ bons, total }) {
  const approved    = bons.filter((b) => b.status === 'APPROVED').length
  const pending     = bons.filter((b) => ['PENDING_PROFORMA', 'PENDING_DAF', 'PENDING_DG'].includes(b.status)).length
  const rejected    = bons.filter((b) => b.status === 'REJECTED').length
  const inExecution = bons.filter((b) => b.status === 'IN_EXECUTION').length
  ...
  const cards = [
    ...
    },
    {
      label: 'Approuvés',
      value: approved,
      sub: `dont ${inExecution} en exécution`,
      bg: 'bg-[#163d6e]',
      icon: (
        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
